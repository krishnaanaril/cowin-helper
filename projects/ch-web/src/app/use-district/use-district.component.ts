...
export class UseDistrictComponent implements OnInit {
  ...
  onSubmit() {    
    ...
    if (searchData.isForWeek) {
      this.dataService.searchAvailabilityByDistrictForWeek(searchData.selectedDistrict.district_id, dateString).subscribe((res) => console.log(res));
    } else {
      this.dataService.searchAvailabilityByDistrict(searchData.selectedDistrict.district_id, dateString).subscribe((res) => console.log(res));
    }    
  }
